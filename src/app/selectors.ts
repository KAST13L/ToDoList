...
): TodolistDomainType[] => state.todolists;
...
export const selectStatus = (state: AppRootStateType): RequestStatusType =>
  state.app.status;
export const selectError = (state: AppRootStateType): null | string =>
  state.app.error;
export const selectSuccess = (state: AppRootStateType): null | string =>
  state.app.success;
